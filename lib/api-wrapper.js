...
export async function apiCall(apiId, params, signal) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      // AbuseIPDB: API key 放在 X-Key header
      // Safe Browsing: API key 追加到 URL query (?key=xxx)
      // 其他: API key 放在 Authorization header
      let finalParams = params ? { ...params } : null
      if (cfg.key) {
        const keyValue = process.env[cfg.key]
        if (keyValue) {
          if (apiId === 'abuseipdb') {
            headers['Key'] = keyValue
          } else if (apiId === 'safebrowsing') {
            finalParams = finalParams || {}
            finalParams.key = keyValue
          } else {
